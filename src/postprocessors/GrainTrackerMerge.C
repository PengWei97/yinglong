...
registerMooseObject("yinglongApp", GrainTrackerMerge);

InputParameters
GrainTrackerMerge::validParams()
{
  InputParameters params = GrainTrackerNew::validParams();
  params.addClassDescription(
      "Grain Tracker derived object for merging of grains based on misorientation angle.");
...
GrainTrackerMerge::GrainTrackerMerge(const InputParameters & parameters)
  : GrainTrackerNew(parameters), _euler(getUserObject<EulerAngleProvider>("euler_angle_provider"))
{
}
          ...
          if (i < j && grain1._id == grain2._id)
          {
            if (grain1._var_index != grain2._var_index)
            {
              if (_verbosity_level > 0)
                _console << COLOR_YELLOW << "Re-merge Grain (#" << grain1._id
