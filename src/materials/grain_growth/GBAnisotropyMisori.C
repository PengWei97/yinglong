...
GBAnisotropyMisori::validParams()
{
  ...
  params.addParam<Real>(
      "TT1_sigma",
      0.9,
      "Twin boundary energy for {10-12} tensile twin (type 1) based on MD, J/m^2");
  params.addParam<Real>(
      "CT1_sigma",
      0.9,
      "Twin boundary energy for {11-22} compresssion twin (type 1) based on MD, J/m^2");
  params.addParam<Real>(
      "TT1_mob",
      2.5e-6,
      "Twin boundary mobility for {10-12} tensile twin (type 1) based on experiment, m^4/(J*s)");
  params.addParam<Real>("CT1_mob",
                        2.5e-6,
                        "Twin boundary mobility for {11-22} compresssion twin (type 1) based on "
                        "experiment, m^4/(J*s)");
  params.addParam<Real>(
      "Sigma9_sigma",
      0.9,
  ...
  params.addParam<Real>(
      ...
      2.5e-6,
      "Twin boundary mobility for {10-12} tensile twin (type 1) based on experiment, m^4/(J*s)");
  params.addParam<Real>("Sigma3_mob",
                        2.5e-6,
                        "Twin boundary mobility for {11-22} compresssion twin (type 1) based on "
                        "experiment, m^4/(J*s)");
  params.addRequiredParam<UserObjectName>(
      "grain_tracker",
      "Name of GrainTracker user object that provides Grain ID according to element ID");
  params.addRequiredParam<UserObjectName>("euler_angle_provider",
                                          "Name of Euler angle provider user object");
  params.addParam<bool>(
      "gb_energy_anisotropy",
      false,
      "The GB energy anisotropy based on misorientation would be considered if true");
  params.addParam<bool>(
      "gb_mobility_anisotropy", true, "The GB mobility anisotropy would be considered if true");
  return params;
...
GBAnisotropyMisori::computeGBProperties()
{
  ...
  if (grain_id_index.size() > 1)
  {
    ...
    // Traverse to obtain the sigma_ij, mob_ij of the activation order parameters at the orthogonal
    // point
    for (unsigned int i = 0; i < grain_id_index.size() - 1; ++i)
    {
      ...
      for (unsigned int j = i + 1; j < grain_id_index.size(); ++j)
      {
        ...
        _misori_s = MisoriAngleCalculator::calculateMisorientaion(
            angles_i, angles_j, _misori_s, CrystalType::FCC);
...
GBAnisotropyMisori::calculatedGBEnergy(const MisorientationAngleData & misori_s)
{
  ...
  if ((_misori_s._misor <= 1.0) && !misori_s._is_twin)
    gbSigma = _GBsigma_HAGB *
              ((2.0 / trans_misori_angle_HAGB * (1 - std::log(2.0 / trans_misori_angle_HAGB))));
  else if ((_misori_s._misor <= trans_misori_angle_HAGB) && !misori_s._is_twin)
    gbSigma = _GBsigma_HAGB * ((misori_s._misor / trans_misori_angle_HAGB *
                                (1 - std::log(misori_s._misor / trans_misori_angle_HAGB))));
  else if ((_misori_s._misor > trans_misori_angle_HAGB) && !misori_s._is_twin)
    gbSigma = _GBsigma_HAGB * ((misori_s._misor / trans_misori_angle_HAGB *
                                (1 - std::log(trans_misori_angle_HAGB / trans_misori_angle_HAGB))));
...
GBAnisotropyMisori::calculatedGBMobility(const MisorientationAngleData & misori_s)
{
  ...
  if ((_misori_s._misor <= 1.0) && !misori_s._is_twin)
    ...
    gbMob = _GBmob_HAGB *
            ((1 - std::exp(-B * std::pow(misori_s._misor / trans_misori_angle_HAGB, n)))); // Eq.8
  else if ((_misori_s._misor > trans_misori_angle_HAGB) && !misori_s._is_twin)
    gbMob = _GBmob_HAGB *
            ((1 - std::exp(-B * std::pow(trans_misori_angle_HAGB / trans_misori_angle_HAGB,
                                         n)))); // Eq.8
  else if (misori_s._is_twin)
  {
    if (misori_s._twin_type == TwinType::TT1_HCP)
