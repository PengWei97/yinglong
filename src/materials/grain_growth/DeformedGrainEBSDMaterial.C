...
DeformedGrainEBSDMaterial::validParams()
{
  ...
  params.addParam<Real>(
      "rho_end_l2",
      2.10e12,
      "Dislocation density of grains after long-term concurrent recovery in level 2");
  params.addParam<Real>(
      "rho_end_l3",
      1.8e13,
      "Dislocation density of grains after long-term concurrent recovery in level 3");
  params.addParam<Real>("rho_critical", 3.9e13, "Critual dislocation density of grains");
  params.addParam<Real>(
      "a_rho_l2",
      4.6e-4,
      "Evolution coefficient of dislocation during medium time recovery at level 2");
  params.addParam<Real>(
      "a_rho_l3",
      6.0e-4,
      "Evolution coefficient of dislocation during medium time recovery at level 3");
...
DeformedGrainEBSDMaterial::getGNDsFromEBSD(const unsigned int & grain_id)
{
  ...
  if (grain_id < _GNDs_provider.getGrainNum())
    rho_init = _GNDs_provider.getAvgData(grain_id)._custom[0] *
               (_length_scale * _length_scale); // GNDs for each grain, 1/m^2
  ...
  if (_concurrent_recovery)
  {
    if (rho_init >= rho_end_l3) // yellow grain
      rho_i = (rho_init - rho_end_l3) * std::exp(-_a_rho_l3 * time_current) +
              rho_end_l3;                                          // func1 - level 3
    else if ((rho_init > rho_end_l2) && (rho_init < rho_critical)) // blue grain
      rho_i = (rho_init - rho_end_l2) * std::exp(-_a_rho_l2 * time_current) +
              rho_end_l2; // func2 - level 2
    else
      rho_i = rho_init;
  }
